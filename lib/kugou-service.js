...
import { vend, vendorRoot } from './vendor.js'
...
class KugouService extends BaseService {
  ...
  async _doStart(config) {
    ...
    const tryStart = async () => {
      // 确保酷狗依赖链接到 vendor 内环境（避免根项目 pnpm 补丁污染）
    try {
      const nmDir = path.join(kugouRoot, 'node_modules')
      fs.mkdirSync(nmDir, { recursive: true })
      const vendorNm = path.join(vendorRoot, 'node_modules')
      const links = ['express','axios','big-integer','crypto-js','dotenv','node-forge','pako','qrcode','safe-decode-uri-component','url']
      for (const dep of links) {
        const target = path.join(vendorNm, dep)
        const linkPath = path.join(nmDir, dep)
        if (!fs.existsSync(target)) continue
        let needLink = true
        try {
          needLink = fs.realpathSync(linkPath) !== fs.realpathSync(target)
        } catch {}
        if (needLink) {
          try { fs.rmSync(linkPath, { recursive: true, force: true }) } catch {}
          try { fs.symlinkSync(path.relative(nmDir, target), linkPath, 'dir') } catch {}
        }
      }
    } catch {}

    const serverModule = vend(path.join(kugouRoot, 'server.js'))

...
import { vendorRoot } from './vendor.js'
...
async function installKugouDeps() {
  try {
    const nmDir = path.join(kugouPath, 'node_modules')
    ...
    for (const dep of links) {
      ...
      await fs.promises.rm(linkPath, { recursive: true, force: true }).catch(() => {})
      await fs.promises.symlink(path.relative(nmDir, target), linkPath, 'dir').catch(() => {})
    }
    return { success: true }
  } catch (err) {

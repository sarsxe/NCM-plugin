...
function logInfo(message) {
  ...
  ;(logger.info || logger.log || console.log).call(logger, '[NCMApi-plugin] ' + message)
}

function logError(message, err) {
  const logger = getLogger()
  ;(logger.error || logger.warn || logger.log || console.error).call(logger, '[NCMApi-plugin] ' + message)
  if (err) {
...
export async function verifyAndSetMaster(userId, inputCode) {
  ...
  try {
    ...
    return { success: true, message: '设置主人完成，您现在拥有 NCMApi 管理权限' }
  } catch (err) {

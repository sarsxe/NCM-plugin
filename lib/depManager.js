...
import { vendorInstall, vendorInstalledVersion } from './vendor.js'
...
function getDepPkgPath() {
  return path.join(pluginPath, 'vendor', 'node_modules', 'NeteaseCloudMusicApi', 'package.json')
}
...
export async function installVersion(version) {
  ...
  try {
    const r = await vendorInstall('NeteaseCloudMusicApi@' + targetVersion)

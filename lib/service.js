...
function loadConfig() {
  try {
    ...
    console.warn('[' + PLUGIN_NAME + '] 读取配置文件失败:', err.message)
  }
  return {}
}
...
function saveConfig(config) {
  try {
    ...
    console.error('[' + PLUGIN_NAME + '] 保存配置文件失败:', err.message)
  }
}
...
function resolveConfig(options = {}) {
  ...
  const host = options.host || baseConfig.host || savedConfig.host || DEFAULT_HOST
  const port = normalizePort(options.port || baseConfig.port || savedConfig.port || DEFAULT_PORT, DEFAULT_PORT)
  return { host, port }
}

export function saveNcmApiConfig(config) {
  const current = loadConfig()
  if (config.port !== undefined) current.port = config.port
  if (config.host !== undefined) current.host = config.host
  saveConfig(current)
}
...
export async function startNcmApiService(options = {}) {
  ...
  state.starting = (async () => {
    try {
      const ncmInfo = detectNcmApiVersion()

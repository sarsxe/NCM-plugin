...
import { vend, ensureVendor } from './vendor.js'
...
async function healRootDeps() {
  const r = await ensureVendor({ force: true })
  if (!r.success) return { success: false, error: r.error }
  if (require.cache) {
    const vr = path.join(pluginRoot, 'vendor', 'node_modules')
    for (const key of Object.keys(require.cache)) {
      if (key.startsWith(vr)) delete require.cache[key]
    }
  }
...
function detectNcmApiVersion() {
  try {
    const ncm = vend('NeteaseCloudMusicApi')
  ...
  state.starting = (async () => {
    try {
      ...
      if (ncmInfo.version === 4) {
        ...
        try {
          vend('NeteaseCloudMusicApi/util/apicache').options({ enabled: false })
        } catch {}

...
export async function ensureVendor({ force = false } = {}) {
  ...
  const ret = await runCmd('npm install --omit=dev --no-audit --no-fund --loglevel=error ' + specList, vendorRoot)
  if (ret.error) {
...
export async function vendorInstall(spec) {
  ...
  const ret = await runCmd('npm install --omit=dev --no-audit --no-fund --loglevel=error ' + name + '@' + version, vendorRoot)
  if (ret.error) {
    log('error', 'vendor 依赖安装失败: ' + (ret.stderr || ret.error.message))
    return { success: false, error: ret.stderr || String(ret.error) }
  }

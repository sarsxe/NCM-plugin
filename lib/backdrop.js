 ...
 *
 * @returns {Promise<{ data: string, isLandscape: boolean, localFile?: string }>}
 *   data 为 data:image/png;base64,... 或 本地图片路径 或 空字符串（渐变兜底）
 *   localFile 仅本地模式返回：图片绝对路径（供 #原图 转 PNG 发图使用）
 */
export async function getBackground() {
  ...
  if (local.path) {
    ...
    return { data: local.path, isLandscape: local.isLandscape, localFile: local.localFile, ratio: local.ratio }
  }

  // 3. 渐变兜底（模板内置渐变背景）
  logger.warn('[NCM-plugin] 无可用背景图，使用渐变兜底背景')
  return { data: '', isLandscape: false, ratio: 0 }
}
...
async function fetchRemoteBackdrop(url, timeout) {
  ...
  try {
    ...
    const ratio = size && size.height > 0 ? size.width / size.height : 0
    logger.info('[NCM-plugin] 远程背景图 ' + sizeKB + 'KB ' + elapsed + 'ms ' + (isLandscape ? '横图' : '竖图') + ' 比例:' + ratio.toFixed(2))

    return { data: 'data:image/png;base64,' + png.toString('base64'), isLandscape, ratio }
  } finally {
    clearTimeout(timer)
  }
}
...
function getLocalBgList() {
  ...
  bgListCache = files.map(name => {
    let isLandscape = false, ratio = 0
    try {
      const size = getImageSize(fs.readFileSync(join(BG_DIR, name)))
      isLandscape = toLandscape(size)
      ratio = size && size.height > 0 ? size.width / size.height : 0
    } catch {}
    return { name, isLandscape, ratio }
  })
  bgListCacheTime = now
  return bgListCache
}
...
function getLocalBackdrop(backdropDefault) {
  ...
  return {
    ...
    localFile: join(BG_DIR, item.name),
    ratio: item.ratio
  }
}

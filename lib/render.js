 ...
 * @returns {Promise<{img: any, backdrop: string, localFile?: string}|null>} 失败返回 null
 */
export async function renderCard(tplName, data = {}) {
  try {
    ...
    const img = await puppeteer.screenshot(PLUGIN_NAME + '/' + tplName, {
      ...
      style: bg.ratio > 0 ? `:root { --bg-ratio: ${bg.ratio}; }` : '',
      ...data
    })
    return img ? { img, backdrop: bg.data, localFile: bg.localFile } : null
  } catch (err) {
...
export async function replyCard(e, tplName, data = {}) {
  ...
  const retMsgId = await e.reply(result.img, false)
  if (retMsgId && result.backdrop) saveOrigImg(e, retMsgId, result.backdrop, result.localFile)
  return true
}

/**
 * 保存背景原图到 redis（关联消息ID，回复 #原图 时取用）
 * 本地图保存 localfile:// 标记 + 绝对路径，回发时读取文件转 PNG
 */
export function saveOrigImg(e, retMsgId, backdrop, localFile) {
  const redisData = localFile ? 'localfile://' + localFile : toOrigImgData(backdrop)
  if (!redisData) return
